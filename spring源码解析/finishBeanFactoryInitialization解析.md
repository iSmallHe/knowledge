...
5. preInstantiateSingletons进入初始化single bean
``` java
...
4. 如果该bean实现了接口SmartInitializingSingleton，则进行初始化后的用户操作。
...
2. getSingleton获取已初始化的bean，如果sharedInstance不为空，则args参数为空，则标识获取
3. 如果未初始化，则进行初始化流程。
